...
| sstable_filter_kind | Per-level xor filter family for compaction output. Index by LSM level: `0..=max_level`. Note: L0 (index 0) is currently ignored by shared-buffer flush, which always uses "xor16". | ["xor16", "xor16", "xor16", "xor16", "xor16", "xor16", "xor16"] |
| sstable_filter_layout | Per-level xor filter layout for compaction output. "auto" uses the kv-count heuristic; "plain"/"normal" forces non-blocked filters; "blocked" forces block-based filters. Explicit "plain" and "blocked" values ignore the kv-count threshold. Index by LSM level: `0..=max_level`. Note: L0 (index 0) is currently ignored by shared-buffer flush, which always uses "auto". | ["auto", "blocked", "blocked", "blocked", "blocked", "blocked", "blocked"] |
...
| enable_kv_log_store_v3 | Enable chunk-level blob WAL for overflowed sink kv log store chunks. | true |
| kv_log_store_blob_prefetch_depth | Number of flushed blob WAL chunks to prefetch concurrently in the sink kv log store reader. Only effective when `enable_kv_log_store_v3` is true. Set to 0 to disable prefetching and read blobs serially. | 8 |
| kv_log_store_buffer_size | The max in-memory row count for sink kv log store, before overflowed chunks are flushed. | 65536 |
