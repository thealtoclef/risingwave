...
impl SpannerExternalTableReader {
    /// Returns the split column based on the backfill_split_pk_column_index.
    ///
    /// Follows Postgres CDC's split_column pattern.
    fn split_column(&self, options: &CdcTableSnapshotSplitOption) -> ConnectorResult<(String, DataType)> {
        let split_column_idx = options.backfill_split_pk_column_index as usize;
        ...
        Ok((
            self.pk_names[split_column_idx].clone(),
            self.pk_types[split_column_idx].clone(),
        ))
    }
    ...
    ///
    /// Fetches `backfill_num_rows_per_split` rows and returns the last row's PK value
    /// as the split boundary.
    async fn next_split_right_bound_exclusive(
        ...
        split_column_name: &str,
        is_first_split: bool,
    ) -> ConnectorResult<Option<Option<String>>> {
        let sql = if is_first_split {
            // First split: no left bound
            format!(
                "SELECT `{}` FROM `{}` ORDER BY `{}` ASC LIMIT {}",
                split_column_name, self.table_name, split_column_name, max_split_size
            )
        } else {
    ...
    async fn next_greater_bound(
        ...
        let greater_sql = format!(
            ...
            self.table_name,
            split_column_name,
            escape_string(start_offset),
            split_column_name,
            escape_string(max_value)
        );

        let greater_stmt = Statement::new(&greater_sql);
        let mut greater_rows = txn
            .query(greater_stmt)
            .await
        ...
        {
            Ok(Some(row.column_by_name(split_column_name).ok().flatten()))
        } else {
            Ok(Some(None))
        }
    }
    ...
    async fn as_even_splits(&self, options: CdcTableSnapshotSplitOption) {
        let (split_column_name, split_column_type) = self.split_column(&options)?;
        ...
        let Some((min_val, max_val)) = self.min_and_max(&mut txn, &split_column_name).await? else {
        ...
        loop {
            ...
            if try_increase_split_id(&mut split_id).is_err() {
                return Err(anyhow::anyhow!("too many CDC snapshot splits").into());
            }
            yield split;
    ...
    async fn as_uneven_splits(&self, options: CdcTableSnapshotSplitOption) {
        let (split_column_name, _split_column_type) = self.split_column(&options)?;
        ...
        let Some((min_value, max_value)) = self.min_and_max(&mut txn, &split_column_name).await?
        else {
            // Table is empty, return a single empty split
            yield CdcTableSnapshotSplit {
                split_id: 1,
        ...
        loop {
            let left_bound_inclusive: Option<String> =
            ...
            let is_first_split = left_bound_inclusive.is_none();
            ...
            if let Some(Some(ref inner)) = next_right {
                let left_val = left_bound_inclusive.as_ref().unwrap_or(&min_value);
                if inner.as_str() == left_val.as_str() {
    ...
    async fn get_parallel_cdc_splits_inner(&self, cdc_split_options: CdcTableSnapshotSplitOption) {
        let backfill_num_rows_per_split = cdc_split_options.backfill_num_rows_per_split;
        ...
        let (split_column_name, split_column_type) = self.split_column(&cdc_split_options)?;
        let row_stream = if cdc_split_options.backfill_as_even_splits
