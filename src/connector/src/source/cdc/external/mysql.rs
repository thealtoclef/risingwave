...
use crate::parser::{mysql_datum_to_rw_datum, mysql_row_to_owned_row_with_strict_pk};
...
impl MySqlExternalTableReader {
    ...
    fn scalar_to_mysql_value(
        ...
        let val = match data_type {
            ...
            _ => bail!(
                "unsupported data type for MySQL bind parameter: {}",
                data_type
            ),
        };
        Ok(val)
    ...
    async fn next_split_right_bound_exclusive(
        ...
        let col = Self::quote_column(&split_column.name);
        let is_unsigned = self.needs_unsigned_i64_compare(&split_column.name)?;
        let sql = format!(
        ...
        if let Some(start_pk_row) = start_pk_row {
            ...
            let params: Vec<_> = primary_keys
                ...
                .map(|(pk, datum)| {
                    ...
                    let val = Self::scalar_to_mysql_value(
                        value,
                        ty,
