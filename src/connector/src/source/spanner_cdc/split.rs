...
///
/// **Offset semantics:**
/// - `offset_as_micros()` returns the lagging edge: `min(root, non-finished children)`.
///   This means "everything before this timestamp has been consumed."
...
impl SpannerCdcSplit {
    ...
    /// Returns the lagging-edge offset: `min(root, non-finished children)`.
    /// Means "everything before this timestamp has been consumed."
    /// Falls back to `self.offset` when all partitions are finished or none tracked.
    pub fn offset_as_micros(&self) -> i64 {
        ...
        for progress in self.partition_progress.values() {
            if progress.state != PartitionState::Finished {
                if let Some(off) = progress.offset {
                    match min_offset {
...
impl SplitMetaData for SpannerCdcSplit {
    ...
    fn encode_to_json(&self) -> JsonbVal {
        ...
        let lagging_edge_micros = self.offset_as_micros();
        let mut persisted = self.clone();
        if lagging_edge_micros > 0 {
            if let Ok(ts) =
                OffsetDateTime::from_unix_timestamp_nanos((lagging_edge_micros as i128) * 1000)
            {
...
mod tests {
    ...
    fn test_lagging_edge_min() {
        ...
        assert_eq!(split.offset_as_micros(), (t50.unix_timestamp_nanos() / 1000) as i64);

        split.partition_progress.get_mut("slow").unwrap().state = PartitionState::Finished;
        assert_eq!(split.offset_as_micros(), (t100.unix_timestamp_nanos() / 1000) as i64);

        split.partition_progress.get_mut("fast").unwrap().state = PartitionState::Finished;
        assert_eq!(split.offset_as_micros(), (t200.unix_timestamp_nanos() / 1000) as i64);
