...
//! - Heartbeat records advance the watermark even when no data changes occur
...
pub struct PartitionInfo {
    ...
    /// Start timestamp for this partition
...
/// - Uses watermark-based scheduling to maintain timestamp ordering
...
impl PartitionManager {
    ...
    pub async fn add_child_partition(
        ...
        self.add_child_partition_internal(
            ...
            parent_tokens,
            start_timestamp,
            start_timestamp, // watermark starts at start_timestamp
            split_id,
        )
        .await
    }

    /// Internal method to add or restore a child partition with a specific watermark.
    ///
    /// This is used both for:
    /// 1. Newly discovered child partitions (watermark = start_timestamp)
    /// 2. Restored child partitions from checkpoint (watermark from checkpoint)
    async fn add_child_partition_internal(
        ...
        // Update watermark if different from start_timestamp (checkpoint restoration)
    ...
    /// Mark a partition as finished and update its watermark.
    ...
    /// 2. The child's start_timestamp is >= the current watermark
    ...
    /// - AND the child's start_timestamp is >= the current minimum watermark
    ///
    ...
    pub async fn get_ready_children(&self) -> Vec<String> {
        ...
        // Get the minimum watermark across all unfinished partitions
        // This is the "safe" timestamp - we can schedule children that
        // start at or after this timestamp
        let min_watermark = self.get_min_watermark_unlocked(&partitions);
        ...
        for (token_key, info) in partitions.iter() {
            if info.state == PartitionState::Created {
                if let Some(token) = token_key {
                    ...
                    // Watermark-based scheduling: child's start_timestamp must be >= min_watermark
                    // This ensures timestamp-ordered processing as per Spanner change stream docs
                    let watermark_ok = info.start_timestamp >= min_watermark;

                    if parents_finished && watermark_ok {
    ...
    /// Get the minimum watermark across all active/unfinished partitions.
    ///
    /// This represents the "safe" timestamp up to which all data has been
    /// processed. Child partitions can only start when their start_timestamp
    /// is >= this minimum watermark.
    fn get_min_watermark_unlocked(&self, partitions: &HashMap<Option<String>, PartitionInfo>) -> OffsetDateTime {
        ...
        for info in partitions.values() {
            ...
            if info.state == PartitionState::Running || info.state == PartitionState::Finished {
                match min_watermark {
                    None => min_watermark = Some(info.watermark),
                    Some(current_min) if info.watermark < current_min => {
                        min_watermark = Some(info.watermark)
                    }
                    _ => {}
                }
            }
        ...
        min_watermark.unwrap_or_else(|| {
            // Default to Unix epoch if no partitions are active
    ...
    /// Get the current watermark (minimum of all active partitions).
    ///
    /// This represents the "safe" timestamp up to which all data has been
    /// processed across all active partitions.
    pub async fn get_watermark(&self) -> OffsetDateTime {
        let partitions = self.partitions.lock().await;
        self.get_min_watermark_unlocked(&partitions)
    }
    ...
    pub async fn debug_info(&self) -> String {
        ...
        for (token, partition_info) in sorted_partitions {
            result.push_str(&format!(
                "  {:?}: state={:?}, start={:?}, watermark={:?}, parents={:?}\n",
                token, partition_info.state, partition_info.start_timestamp, partition_info.watermark, partition_info.parent_tokens
            ));
        }
...
mod tests {
    ...
    async fn test_root_partition() {
        ...
        manager.initialize_root(start, split_id).await;
    ...
    async fn test_child_partition_wait_for_parents() {
        ...
        // Add root
        manager.initialize_root(start, "root".into()).await;
    ...
    async fn test_child_partition_with_multiple_parents() {
        ...
        manager
            .mark_finished(&Some("parent1".to_string()), start)
            .await;
        ...
        manager
            .mark_finished(&Some("parent2".to_string()), start)
            .await;
    ...
    #[tokio::test]
    async fn test_watermark_based_scheduling() {
        let manager = PartitionManager::new();
        ...
        // Mark root as running and update its watermark
        manager.mark_running(&None).await;
        manager
            .mark_finished(&None, OffsetDateTime::from_unix_timestamp(1500).unwrap())
            .await;

        // Add a child partition that starts AFTER the root's watermark
        manager
            .add_child_partition(
                "child1".to_string(),
                vec![],
                later_start, // start=2000, which is >= root watermark=1500
                "child1".into(),
            )
            .await;
        ...
        // 2. start_timestamp (2000) >= min_watermark (1500)
        let ready = manager.get_ready_children().await;
        assert_eq!(ready, vec!["child1".to_string()]);
    }

    #[tokio::test]
    async fn test_watermark_blocks_child_scheduling() {
        let manager = PartitionManager::new();
        let start = OffsetDateTime::from_unix_timestamp(1000).unwrap();
        ...
        // Mark root as running but with low watermark
        manager.mark_running(&None).await;
        manager
            .mark_finished(&None, OffsetDateTime::from_unix_timestamp(1100).unwrap())
            .await;

        // Add a child partition that starts BEFORE the current min watermark
        // start=1200, min_watermark=1100
        ...
        // Child should be ready because start (1200) >= min_watermark (1100)
        let ready = manager.get_ready_children().await;
        assert_eq!(ready, vec!["child1".to_string()]);

        // Now add another child with start timestamp in the "future" relative to watermark
        // and verify it's NOT ready until watermark advances
        ...
        // child2 should NOT be ready because start (2000) > min_watermark (1100)
        let ready = manager.get_ready_children().await;
    ...
    #[tokio::test]
    async fn test_child_with_multiple_parents_watermark_check() {
        let manager = PartitionManager::new();
        let start = OffsetDateTime::from_unix_timestamp(1000).unwrap();
        ...
        // Mark first parent as finished with low watermark
        manager
            .mark_finished(
                &Some("parent1".to_string()),
                OffsetDateTime::from_unix_timestamp(1500).unwrap(),
            )
            .await;
        ...
        manager
            .mark_finished(
                ...
                OffsetDateTime::from_unix_timestamp(1600).unwrap(),
            )
            .await;

        // Now child should be ready:
        // - All parents finished
        // - child start (2000) >= min_watermark (1500)
        let ready = manager.get_ready_children().await;
        assert!(ready.contains(&"child".to_string()));
    }
}
