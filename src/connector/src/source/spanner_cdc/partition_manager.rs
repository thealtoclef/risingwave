...
impl PartitionManager {
    ...
    /// Mark a partition as scheduled (queued but not yet running).
    /// Used to prevent get_ready_children() from re-returning a deferred partition.
    pub async fn mark_scheduled(&self, partition_token: &Option<String>) {
        let mut partitions = self.partitions.lock().await;
        if let Some(info) = partitions.get_mut(partition_token) {
            info.state = PartitionState::Scheduled;
        }
    }
