...
When all partitions are finished and buffer is non-empty, returns `OffsetDateTime::MAX` to force-drain everything remaining.
...
| Runtime coordination | ReorderBuffer + sibling groups + schema | No (recreated on restart) |
...
| `source/reader.rs` | CDC streaming reader — partition tasks → shared channel → ReorderBuffer → ordered emission |
