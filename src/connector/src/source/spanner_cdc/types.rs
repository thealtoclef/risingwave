...
pub fn parse_change_record_column(
    ...
    elements
        ...
        .map(|inner| match inner {
            JsonValue::Object(obj) => ChangeStreamRecord::from_json_object(obj),
...
impl Mod {
    ...
    /// rather than copied.
    ///
    /// Only the cells `mod_type` reads in [`Mod::to_json_map`] are parsed; the others are left
    /// `None` unexamined, so a malformed cell that is never used cannot fail the record.
    fn from_json(v: JsonValue, mod_type: &str) -> anyhow::Result<Self> {
        let mut obj = match v {
            JsonValue::Object(obj) => obj,
        ...
        Ok(Self {
            keys: take("keys")?,
            new_values: if uses_new { take("new_values")? } else { None },
            old_values: if uses_old { take("old_values")? } else { None },
        })
    }
}
...
impl Mod {
    ...
    pub fn to_json_map(&self, mod_type: &str, column_types: &HashMap<&str, TypeCode>) -> CellMap {
        ...
        result.insert("before".to_owned(), before);
        result.insert("after".to_owned(), after);
        result.insert("op".to_owned(), JsonValue::String(String::from(op)));
...
mod tests {
    ...
    fn test_mod_from_json_absent_cells() {
        ...
        let m = Mod::from_json(serde_json::json!({"keys": {"id": "1"}}), "UPDATE").unwrap();
        assert!(m.new_values.is_none());
        assert!(m.old_values.is_none());
    }

    /// A malformed cell the mod type never reads must not fail the record, or the source
    /// would retry the same row forever.
    #[test]
    fn test_mod_from_json_ignores_unused_cells() {
        let m = Mod::from_json(
            serde_json::json!({"keys": {"id": "1"}, "new_values": 42, "old_values": {"v": "1"}}),
            "DELETE",
        )
        .unwrap();
        assert!(m.new_values.is_none());
        assert_eq!(m.old_values, cells(r#"{"v":"1"}"#));

        let m = Mod::from_json(
            serde_json::json!({"keys": {"id": "1"}, "new_values": {"v": "1"}, "old_values": 42}),
            "INSERT",
        )
        .unwrap();
        assert!(m.old_values.is_none());

        // A cell the mod type does read is still validated.
        assert!(
            Mod::from_json(
                serde_json::json!({"keys": {"id": "1"}, "old_values": 42}),
                "UPDATE",
            )
            .is_err()
        );
    ...
    fn test_mod_from_json_rejects_non_object_cell() {
        let err = Mod::from_json(serde_json::json!({"keys": 42}), "INSERT").unwrap_err();
        assert!(err.to_string().contains("keys"), "unexpected error: {err}");

        let err = Mod::from_json(serde_json::json!({"keys": "not json"}), "INSERT").unwrap_err();
        assert!(err.to_string().contains("keys"), "unexpected error: {err}");
    }
