...
async fn execute_query(
    ...
    // Per-partition schema tracker — no locks, no sharing.
    let mut schema_tracker = SchemaTracker::new();
        ...
        .map_err(|e| anyhow::anyhow!("failed to get next row: {}", e))?
    {
        ...
        for record in change_records {
            ...
            for data_change in &record.data_change_record {
                ...
                if let Some(schema_payload) = schema_tracker.check_and_evolve(data_change) {
                    if !messages.is_empty() {
