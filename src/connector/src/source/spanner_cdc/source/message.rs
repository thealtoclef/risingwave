...
pub fn build_source_message(
    ...
    source.insert("ts_ms".to_owned(), serde_json::Value::from(source_ts_ms));
    source.insert(
        "db".to_owned(),
    ...
    inner.insert("source".to_owned(), serde_json::Value::Object(source));
    let mut envelope = serde_json::Map::new();
    envelope.insert("payload".to_owned(), serde_json::Value::Object(inner));
    let mod_json =
        serde_json::to_vec(&envelope).expect("serializing a JSON object to bytes is infallible");

    SourceMessage {
        key: None,
        payload: Some(mod_json),
