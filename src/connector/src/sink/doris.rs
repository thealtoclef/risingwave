...
use super::doris_starrocks_connector::{
    DORIS_DELETE_SIGN, HeaderBuilder, InserterInnerBuilder, POOL_IDLE_TIMEOUT,
};
use super::{
...
/// Default cap on the payload of a single stream load, 32MB. See
...
pub struct DorisCommon {
    ...
    /// Doris 3 target.
    #[serde(rename = "doris.timestamptz_as_datetime")]
    #[serde(default)]
    #[serde_as(as = "DisplayFromStr")]
...
impl DorisCommon {
    ...
    /// required when `create_table_if_not_exists` is enabled or schema change is used, both of which issue DDL via
    /// the FE MySQL port).
    fn get_query_url(&self) -> Result<&str> {
        self.query_url.as_deref().ok_or_else(|| {
            SinkError::DorisStarrocksConnect(anyhow!(
                "doris.query_url must be set when create_table_if_not_exists or schema change is used"
            ))
        })
    }
    ...
    pub fn from_btreemap(properties: BTreeMap<String, String>) -> Result<Self> {
        ...
        if let Some(partition_by) = &config.common.partition_by {
            if !config.common.create_table_if_not_exists {
                return Err(SinkError::Config(anyhow!(
                    "`partition_by` only takes effect when `create_table_if_not_exists` is enabled"
                )));
            }
...
/// Whether appending a row of `row_size` bytes to the pending batch would push it past
/// `max_batch_size_bytes`, splitting the batch into two loads.
///
...
/// Parse a `partition_by` value into a [`DorisPartitionSpec`].
///
...
fn parse_partition_by(partition_by: &str) -> Result<DorisPartitionSpec> {
    ...
    if partition_by.is_empty() {
        return Err(SinkError::Config(anyhow!(
            "`partition_by` must not be empty"
        )));
    }
    ...
    if partition_by.contains('(') {
        let (granularity, column) = partition_by
            ...
            .ok_or_else(|| {
                SinkError::Config(anyhow!(
                    "Invalid `partition_by`: {partition_by:?}\n\
        ...
        let granularity = DORIS_AUTO_PARTITION_GRANULARITIES
            ...
            .ok_or_else(|| {
                SinkError::Config(anyhow!(
                    "`partition_by` granularity must be one of {}, got {:?}",
...
impl DorisSink {
        ...
        match rw_data_type {
            ...
            risingwave_common::types::DataType::Timestamptz => {
                ...
                if doris_data_type.contains("TIMESTAMPTZ")
                    || (timestamptz_as_datetime && doris_data_type.contains("DATETIME"))
                {
                    Ok(true)
                } else {
                    Err(SinkError::Doris(format!(
                        "TIMESTAMP WITH TIMEZONE can only be written to a Doris `TIMESTAMPTZ` \
                         ...
                         `doris.timestamptz_as_datetime = 'true'` to accept storing it in a \
    ...
    /// `Timestamptz` is key-able in both of its target types: native `TIMESTAMPTZ` (Doris 4) and
    /// `DATETIME` under `timestamptz_as_datetime` (a Doris 3 target).
    ...
    /// accepts. Doris auto-RANGE partitions on a `date_trunc` of a `DATE`, `DATETIME`, or
    /// `TIMESTAMPTZ` column (a RisingWave `Date`, `Timestamp`, or `Timestamptz`). A
    /// `Timestamptz` maps to `TIMESTAMPTZ` by default (Doris 4) or `DATETIME` under
    /// `timestamptz_as_datetime` (a Doris 3 target); `date_trunc` accepts both.
    fn is_range_partitionable(data_type: &DataType) -> bool {
        matches!(
            data_type,
    ...
    /// Render the auto-partitioning clause for `partition_by`, if set, validating that
    ...
    ///
    /// Returns `None` when `partition_by` is unset. The tuple is
    ...
    fn build_partition_clause(&self, key_indices: &[usize]) -> Result<Option<(bool, Option<String>, String)>> {
        ...
        for column in &columns {
            let idx = fields
                ...
                .ok_or_else(|| {
                    SinkError::Doris(format!(
                        "`partition_by` names column `{column}`, which does not exist in \
                         the sink schema"
                    ))
                })?;
            if !key_indices.contains(&idx) {
                return Err(SinkError::Doris(format!(
                    "`partition_by` column `{column}` must be a Doris key column: Doris \
            ...
            let type_ok = if is_range {
                Self::is_range_partitionable(data_type)
            } else {
                Self::is_list_partitionable(data_type)
            };
            if !type_ok {
                let allowed = if is_range {
                    "Doris auto-RANGE partitions on a `DATE`, `DATETIME`, or `TIMESTAMPTZ` column \
                     (a RisingWave `Date`, `Timestamp`, or `Timestamptz`)"
                } else {
                    "Doris auto-LIST accepts boolean, integer, date/time, or varchar columns"
                };
                return Err(SinkError::Doris(format!(
                    "`partition_by` column `{column}` of type {data_type:?} cannot be a \
    ...
    fn build_create_table_sql(&self, alive_be_count: u32) -> Result<String> {
        ...
        // the primary key when present; otherwise, when `partition_by` is set, the partition
        // columns become the (duplicate) key — they are the columns Doris requires as the key, so
        // keying on them is the natural choice. With neither a primary key nor a partition, Doris
        // still needs a (duplicate) key, so pick the first key-able column.
        let key_indices: Vec<usize> = if !self.pk_indices.is_empty() {
            ...
            .partition_by
            .as_deref()
            .map(parse_partition_by)
            .transpose()?
        {
            let mut partition_indices = Vec::new();
            let mut missing = Vec::new();
            let mut not_key_able = Vec::new();
            let partition_columns: Vec<String> = match spec {
                DorisPartitionSpec::Range { column, .. } => vec![column],
                DorisPartitionSpec::List { columns } => columns,
            };
            for column in &partition_columns {
                match fields.iter().position(|f| f.name == *column) {
                    Some(idx) if Self::is_doris_key_type(&fields[idx].data_type) => {
                        partition_indices.push(idx);
                    }
                    Some(idx) => {
                        not_key_able.push((column.clone(), fields[idx].data_type.clone()));
                    }
            ...
            if !missing.is_empty() {
                return Err(SinkError::Doris(format!(
                    "`partition_by` names column(s) {:?}, which do not exist in the sink schema",
                    missing
                )));
            }
            if let Some((column, data_type)) = not_key_able.first() {
                return Err(SinkError::Doris(format!(
                    "`partition_by` column `{column}` of type {data_type:?} cannot be used as a \
                     Doris key column (Doris forbids FLOAT/DOUBLE/JSON/complex types as keys). \
                     Please create the table manually or choose a different partition column."
                )));
            }
            partition_indices
        } else {
...
impl SinglePhaseCommitCoordinator for DorisSinkCommitCoordinator {
    async fn init(&mut self) -> Result<()> {
        // Auto-create happens during `validate` (when `create_table_if_not_exists` is set); there is nothing to
        // set up here.
        Ok(())
    }
...
impl Sink for DorisSink {
    ...
    async fn validate(&self) -> Result<()> {
        ...
        if self.config.common.create_table_if_not_exists {
...
pub struct DorisSinkWriter {
    ...
    max_batch_size_bytes: u64,
    /// Rows accumulated for the next JSON load, newline-separated.
    json_pending: Vec<u8>,
    /// Estimated size of `json_pending`, the trigger for splitting a load.
    json_pending_size: u64,
...
impl DorisSinkWriter {
    pub async fn new(
        ...
        // Pick the stream-load base. `doris.stream_load_url` addresses a BE directly; otherwise
        // `doris.url` (an FE) is used and its redirect to a BE is followed on each `send_body`.
    ...
    /// Commit the pending JSON batch as one stream load, if there is one.
    async fn finish_load_request(&mut self) -> Result<()> {
        let bytes = std::mem::take(&mut self.json_pending);
        self.json_pending_size = 0;
        if bytes.is_empty() {
            return Ok(());
        }
    ...
    async fn write_row_json(&mut self, row_json_string: String) -> Result<()> {
        ...
        self.json_pending
            .extend_from_slice(row_json_string.as_bytes());
        self.json_pending.push(b'\n');
        self.json_pending_size = size_decision.next_batch_size_bytes;
        Ok(())
    }
    ...
    async fn append_only(&mut self, chunk: StreamChunk) -> Result<()> {
        ...
        for (op, row) in chunk.rows() {
            ...
            let row_json_string = self.encode_row_json(row, None)?;
    ...
    async fn upsert(&mut self, chunk: StreamChunk) -> Result<()> {
        ...
        for (op, row) in chunk.rows() {
            // `__DORIS_DELETE_SIGN__` drives the upsert delete; the value marks the op.
            let delete_sign = match op {
                Op::Delete => Some("1"),
                Op::Insert | Op::UpdateInsert => Some("0"),
                Op::UpdateDelete => None,
            };
            if let Some(delete_sign) = delete_sign {
                let row_json_string = self.encode_row_json(row, Some(delete_sign))?;
                self.write_row_json(row_json_string).await?;
            }
        }
        Ok(())
    ...
    /// Encode one row as a JSON object, appending the `__DORIS_DELETE_SIGN__` value for upsert
    /// rows, and serialize it.
    fn encode_row_json(&self, row: impl Row, delete_sign: Option<&str>) -> Result<String> {
        let mut row_json_value = self.row_encoder.encode(row)?;
        if let Some(delete_sign) = delete_sign {
            row_json_value.insert(
                DORIS_DELETE_SIGN.to_owned(),
                Value::String(delete_sign.to_owned()),
            );
        }
        serde_json::to_string(&row_json_value)
            .map_err(|e| SinkError::Doris(format!("Json serialize error: {}", e.as_report())))
    }
    ...
    async fn write_arrow_load(&mut self, chunk: StreamChunk) -> Result<()> {
        ...
        for (col, field) in chunk.columns().iter().zip_eq_fast(schema.fields().iter()) {
            ...
            ) = (col.as_ref(), field.data_type())
            {
                ...
                columns.push(Arc::new(
                    arrow_array_58::TimestampMicrosecondArray::try_from(arr).map_err(|e| {
                        SinkError::Doris(format!("arrow encode error: {}", e.as_report()))
                    })?,
                ));
            } else {
                columns.push(
...
impl Drop for DorisSinkWriter {
    fn drop(&mut self) {
        // Pending rows were never sent, so nothing is committed and the epoch is replayed.
        if self.json_pending_size > 0 || self.arrow_pending_size > 0 {
            tracing::warn!(
                json_pending_size = self.json_pending_size,
                arrow_pending_size = self.arrow_pending_size,
                "doris sink writer dropped with rows still buffered; they were not sent and the \
                 affected epochs will be replayed"
            );
        }
    }
}
...
impl DorisSchemaClient {
    ...
    pub async fn get_schema_from_doris(&self) -> Result<DorisSchema> {
        ...
        let schema = parse_doris_schema(json_data, &self.db, &self.table)?;
        Ok(schema)
    }
}

/// Parse the body of a Doris `_schema` response into a [`DorisSchema`]. `keysType` is only
/// reported for OLAP tables; when it is missing, the target exists but is not an OLAP table and
/// cannot be a sink target. Report that clearly instead of a raw serde `missing field keysType`
/// failure.
fn parse_doris_schema(data: Value, db: &str, table: &str) -> Result<DorisSchema> {
    match serde_json::from_value(data) {
        Ok(schema) => Ok(schema),
        Err(err) if err.to_string().contains("missing field `keysType`") => Err(
            SinkError::DorisStarrocksConnect(anyhow!(
                "Target Doris object `{db}.{table}` exists but is not an OLAP table, so it \
                 cannot be used as a Doris sink target. The target must be a Doris OLAP table."
            )),
        ),
        Err(err) => Err(SinkError::DorisStarrocksConnect(anyhow!(
            "Can't get schema from json: {err}"
        ))),
    }
}
...
mod tests {
    ...
    /// A tiny HTTP server that answers the schema `GET` with a valid Doris schema and stream loads
    /// (`PUT`) with a fixed success response, so a whole `DorisSinkWriter::new` plus commits run
    /// against it.
    fn stream_load_mock(
        response: &'static str,
        ...
        let handle = tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    let body = if request.contains("_schema") {
                        r#"{"properties":[
                            {"name":"id","type":"BIGINT","comment":"","aggregation_type":""},
                            {"name":"name","type":"VARCHAR(65533)","comment":"","aggregation_type":""},
                            {"name":"age","type":"INT","comment":"","aggregation_type":""}
                        ],"keysType":"UNIQUE_KEYS","status":200}"#
                            .to_owned()
                    } else {
                        sent.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                        format!(
                            r#"{{"TxnId":1,"Label":"l","Status":"{}","TwoPhaseCommit":"false","Message":"OK","NumberTotalRows":0,"NumberLoadedRows":0,"NumberFilteredRows":0,"NumberUnselectedRows":0,"LoadBytes":0,"LoadTimeMs":1,"BeginTxnTimeMs":1,"StreamLoadPutTimeMs":1,"ReadDataTimeMs":1,"WriteDataTimeMs":1,"CommitAndPublishTimeMs":1}}"#,
                            response
                        )
                    };
                    let _ = stream
    ...
    /// Build a `DorisSinkWriter` whose schema fetch and stream loads go to [`stream_load_mock`].
    /// Returns the writer and the count of stream-load requests the mock received.
    async fn writer_with_http_mock(
        properties: BTreeMap<String, String>,
        response: &'static str,
    ) -> (DorisSinkWriter, std::sync::Arc<std::sync::atomic::AtomicUsize>) {
        let sent = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let (url, _handle) = stream_load_mock(response, sent.clone());
        ...
        let builder = InserterInnerBuilder::new(
            url.to_string(),
            "demo".to_owned(),
            "sink_table".to_owned(),
            DorisSinkWriter::build_stream_load_header(&config, &writer.schema, true),
            30_000,
            true,
        )
        .unwrap();
        writer.inserter_inner_builder = builder;
        (writer, sent)
    }
    ...
    #[test]
    fn test_build_create_table_sql_upsert_timestamptz_pk_ok() {
        // A `timestamptz` is a valid Doris key column in both of its target types: native
        // `TIMESTAMPTZ` (Doris 4) and `DATETIME` under `timestamptz_as_datetime` (a Doris 3
        // target). Auto-create must accept it as the primary key.
        for (flag, expected_type) in [(false, "TIMESTAMPTZ(6)"), (true, "DATETIME(6)")] {
            let mut properties = base_properties("upsert");
            if flag {
                properties.insert("doris.timestamptz_as_datetime".to_owned(), "true".to_owned());
            }
            let config = DorisConfig::from_btreemap(properties).unwrap();
            let schema = Schema::new(vec![
            ...
            let sink =
                DorisSink::new(config, schema, vec![0], false, test_param("upsert", false)).unwrap();
            let sql = sink.build_create_table_sql(1).unwrap();
            assert!(sql.contains("UNIQUE KEY(`updated_at`)"), "sql: {sql}");
            assert!(
                sql.contains(&format!("`updated_at` {expected_type}")),
                "expected `updated_at` {expected_type}, sql: {sql}"
            );
        }
    }
    ...
    /// Build a sink whose config has `partition_by` set (and `create_table_if_not_exists` on), with `id`
    ...
    fn partition_sink(
        ...
        properties.insert("create_table_if_not_exists".to_owned(), "true".to_owned());
        properties.insert("partition_by".to_owned(), partition_by.to_owned());
        let config = DorisConfig::from_btreemap(properties).unwrap();
    ...
    fn test_parse_partition_by_rejects_invalid_forms() {
        for (value, needle) in [
            ("", "must not be empty"),
            ("fortnight(ts)", "granularity must be one of"),
            ...
            ("month(a), b", "Invalid `partition_by`"),
            ("month(a), day(b)", "Invalid `partition_by`"),
    ...
    #[test]
    fn test_partition_by_requires_create_table_if_not_exists() {
        let mut properties = base_properties(SINK_TYPE_APPEND_ONLY);
        properties.insert("partition_by".to_owned(), "id".to_owned());
        let err = DorisConfig::from_btreemap(properties).unwrap_err();
        let msg = format!("{}", err);
        assert!(
            msg.contains("only takes effect when `create_table_if_not_exists` is enabled"),
            "got: {msg}"
        );
    }
    ...
    fn test_partition_by_parsed_at_config_time() {
        ...
        // fails `from_btreemap` even with `create_table_if_not_exists` on.
        let mut properties = base_properties(SINK_TYPE_APPEND_ONLY);
        properties.insert("create_table_if_not_exists".to_owned(), "true".to_owned());
        properties.insert("partition_by".to_owned(), "nope(a)".to_owned());
    ...
    #[test]
    fn test_create_table_sql_append_only_no_pk_uses_partition_columns_as_key() {
        // Append-only with no primary key but with `partition_by`: Doris requires the partition
        // columns to be key columns, so the DUPLICATE KEY becomes those columns (instead of the
        // first key-able column). This keeps the partition column in the key without mutating a
        // user-declared pk, and lets the append-only LIST case still omit `DISTRIBUTED BY`.
        let sink = partition_sink(
            SINK_TYPE_APPEND_ONLY,
            true,
            "month(ts)",
        ...
        let sql = sink.build_create_table_sql(1).unwrap();
        assert!(sql.contains("DUPLICATE KEY(`ts`)"), "sql: {sql}");
        assert!(
            sql.contains("AUTO PARTITION BY RANGE(date_trunc(`ts`, 'month'))"),
            "sql: {sql}"
        );
        // No pk: append-only omits `DISTRIBUTED BY` (RANDOM BUCKETS AUTO).
        assert!(!sql.contains("DISTRIBUTED BY"), "sql: {sql}");
    }

    #[test]
    fn test_create_table_sql_append_only_no_pk_list_partition_uses_partition_columns_as_key() {
        // Bare (LIST) form with no pk: the partition columns become the DUPLICATE KEY, and
        // `DISTRIBUTED BY` is omitted for auto-LIST (RANDOM BUCKETS AUTO).
        let sink = partition_sink(
            SINK_TYPE_APPEND_ONLY,
            true,
            "id, region",
            vec![],
            vec![Field::with_name(DataType::Varchar, "region")],
        );
        let sql = sink.build_create_table_sql(1).unwrap();
        assert!(
            sql.contains("DUPLICATE KEY(`id`, `region`)"),
            "sql: {sql}"
        );
        assert!(
            sql.contains("AUTO PARTITION BY LIST(`id`, `region`)"),
            "sql: {sql}"
        );
        assert!(!sql.contains("DISTRIBUTED BY"), "sql: {sql}");
    }
    ...
    fn test_create_table_sql_range_partition_rejects_non_date_type() {
        // Auto-RANGE partitions on a `date_trunc` of a `DATE`/`DATETIME`/`TIMESTAMPTZ` column;
        // an integer partition column has no valid RANGE reading.
    ...
    #[test]
    fn test_create_table_sql_range_partition_accepts_timestamptz() {
        // A `timestamptz` partition column maps to Doris `TIMESTAMPTZ` (Doris 4) by default, or
        // `DATETIME` under `timestamptz_as_datetime` (a Doris 3 target); `date_trunc` accepts
        // both, so auto-RANGE partitioning must accept the type in either mode.
        for flag in [false, true] {
            let mut properties = base_properties(SINK_TYPE_APPEND_ONLY);
            properties.insert("create_table_if_not_exists".to_owned(), "true".to_owned());
            properties.insert("partition_by".to_owned(), "month(ts)".to_owned());
            if flag {
                properties.insert("doris.timestamptz_as_datetime".to_owned(), "true".to_owned());
            }
            let config = DorisConfig::from_btreemap(properties).unwrap();
            let fields = vec![
                Field::with_name(DataType::Int64, "id"),
                Field::with_name(DataType::Timestamptz, "ts"),
            ];
            let sink = DorisSink::new(
                config,
                Schema::new(fields),
            ...
            .unwrap();
            let sql = sink.build_create_table_sql(1).unwrap();
            let expected_column_type = if flag {
                "DATETIME(6)"
            } else {
                "TIMESTAMPTZ(6)"
            };
            assert!(
                sql.contains(&format!("`ts` {expected_column_type} NOT NULL")),
                "expected `ts` {expected_column_type} NOT NULL, sql: {sql}"
            );
            assert!(
                sql.contains("AUTO PARTITION BY RANGE(date_trunc(`ts`, 'month'))"),
                "sql: {sql}"
    ...
    fn test_get_query_url_errors_when_absent() {
        // When query_url is unset and create_table_if_not_exists is enabled, the helper used by
    ...
    fn test_config_defaults() {
        ...
        assert!(!config.common.create_table_if_not_exists);
    ...
    fn test_config_parses_timestamptz_as_datetime() {
        ...
        on.insert("doris.timestamptz_as_datetime".to_owned(), "true".to_owned());
        let config = DorisConfig::from_btreemap(on).unwrap();
        assert!(config.common.timestamptz_as_datetime);

        let mut bad = base_properties(SINK_TYPE_APPEND_ONLY);
        bad.insert("doris.timestamptz_as_datetime".to_owned(), "yes".to_owned());
    ...
    /// `create_table_if_not_exists` is deliberately *not* prefixed with the connector name, unlike
    /// every other Doris-specific option. It is a cross-connector option name: Doris, `BigQuery`,
    /// Iceberg, Snowflake, and Redshift all spell auto-create the same way.
    #[test]
    fn test_create_table_if_not_exists_is_not_connector_prefixed() {
        let mut properties = base_properties(SINK_TYPE_APPEND_ONLY);
        properties.insert("create_table_if_not_exists".to_owned(), "true".to_owned());
        let config = DorisConfig::from_btreemap(properties).unwrap();
        assert!(config.common.create_table_if_not_exists);
        ...
        prefixed.insert("doris.create_table_if_not_exists".to_owned(), "true".to_owned());
        let config = DorisConfig::from_btreemap(prefixed).unwrap();
        assert!(!config.common.create_table_if_not_exists);
        assert!(config
            .unknown_fields
            .contains_key("doris.create_table_if_not_exists"));
    }
    ...
    #[tokio::test]
    async fn test_finish_load_request_sends_newline_separated_json() {
        let properties = base_properties(SINK_TYPE_APPEND_ONLY);
        let (mut writer, _sent) = writer_with_http_mock(properties, "Success").await;
        writer
            .write_batch(StreamChunk::from_rows(
                &[
                    (Op::Insert, OwnedRow::new(vec![Some(risingwave_common::types::ScalarImpl::Int64(1)), Some(risingwave_common::types::ScalarImpl::Utf8("a".into())), Some(risingwave_common::types::ScalarImpl::Int32(2))])),
                    (Op::Insert, OwnedRow::new(vec![Some(risingwave_common::types::ScalarImpl::Int64(3)), Some(risingwave_common::types::ScalarImpl::Utf8("b".into())), Some(risingwave_common::types::ScalarImpl::Int32(4))])),
                ],
                &[DataType::Int64, DataType::Varchar, DataType::Int32],
            ))
            .await
            .unwrap();
        assert_eq!(writer.json_pending, br#"{"age":2,"id":1,"name":"a"}
{"age":4,"id":3,"name":"b"}
"#);
        assert_eq!(writer.json_pending_size, 56);
        writer.finish_load_request().await.unwrap();
        assert!(writer.json_pending.is_empty());
        assert_eq!(writer.json_pending_size, 0);
    }

    #[tokio::test]
    async fn test_split_sends_two_loads() {
        let mut properties = base_properties(SINK_TYPE_APPEND_ONLY);
        properties.insert("doris.max_batch_size_bytes".to_owned(), "26".to_owned());
        let (mut writer, sent) = writer_with_http_mock(properties, "Success").await;
        // First row (28 bytes) is over the 26-byte cap but buffered alone; the second closes it
        // so the oversized row still ends up in its own load.
        writer
            .write_batch(StreamChunk::from_rows(
                &[(Op::Insert, OwnedRow::new(vec![Some(risingwave_common::types::ScalarImpl::Int64(1)), Some(risingwave_common::types::ScalarImpl::Utf8("a".into())), Some(risingwave_common::types::ScalarImpl::Int32(2))]))],
                &[DataType::Int64, DataType::Varchar, DataType::Int32],
            ...
            .await
            .unwrap();
        assert_eq!(writer.json_pending_size, 28);
        writer
            .write_batch(StreamChunk::from_rows(
                &[(Op::Insert, OwnedRow::new(vec![Some(risingwave_common::types::ScalarImpl::Int64(3)), Some(risingwave_common::types::ScalarImpl::Utf8("b".into())), Some(risingwave_common::types::ScalarImpl::Int32(4))]))],
                &[DataType::Int64, DataType::Varchar, DataType::Int32],
            ...
            .await
            .unwrap();
        assert_eq!(writer.json_pending_size, 28); // only the second row remains buffered
    ...
    #[test]
    fn test_parse_schema_without_keys_type_reports_not_olap() {
        // A non-OLAP target (e.g. a view or external table) is reported by Doris without
        // `keysType`. The raw serde error must be translated into a clear "not an OLAP table"
        // message instead of surfacing `missing field keysType`.
        let json = serde_json::json!({
            "properties": [
                { "name": "id", "type": "INT", "comment": "", "aggregation_type": "" }
            ],
            "status": 200
        });
        let err = parse_doris_schema(json, "demo", "t").unwrap_err();
        let msg = format!("{err}");
        assert!(msg.contains("not an OLAP table"), "got: {msg}");
        assert!(msg.contains("demo.t"), "got: {msg}");
        assert!(!msg.contains("missing field"), "got: {msg}");
    }

    #[test]
    fn test_parse_schema_with_keys_type_parses() {
        let json = serde_json::json!({
            "properties": [
                { "name": "id", "type": "INT", "comment": "", "aggregation_type": "" }
            ],
        ...
        let schema = parse_doris_schema(json, "demo", "t").unwrap();
        assert_eq!(schema.keys_type, "UNIQUE_KEYS");
...
impl DorisInsertResultResponse {
    /// Test-only: verify that Doris did not silently drop rows.
    ///
