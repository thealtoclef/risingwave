...
pub(super) async fn create_table_if_not_exists_impl(
    ...
    if let Some(props_str) = &config.common.namespace_properties {
        for pair in props_str.split(';') {
            ...
            if let Some((key, value)) = pair.split_once('=') {
                namespace_properties.insert(key.trim().to_owned(), value.trim().to_owned());
            }
        }
    }
    ...
    create_namespace_if_not_exists(catalog.as_ref(), &namespace, Some(namespace_properties))
        .await?;
    ...
    if let Some(props_str) = &config.common.table_properties {
        for pair in props_str.split(';') {
            ...
            if let Some((key, value)) = pair.split_once('=') {
                table_properties.insert(key.trim().to_owned(), value.trim().to_owned());
            }
        }
    }
...
async fn create_namespace_if_not_exists(
    ...
    let namespace_properties = namespace_properties.unwrap_or_default();
