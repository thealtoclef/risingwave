...
impl PostgresExternalTable {
    ...
    pub async fn connect(
        ...
        let (columns, pk_names, comments) = Self::discover_pk_and_full_columns(
            username,
            password,
            host,
            port,
...
/// Polls `pg_last_wal_replay_lsn()` on `client` until WAL ≥ `target_lsn` or timeout.
///
/// Uses the *replay* LSN rather than the *receive* LSN: on a hot standby a query's
/// snapshot reflects the last WAL location that has been *replayed*, not merely
/// received. WAL can be received-but-not-yet-replayed for a long time (e.g. under
/// `max_standby_streaming_delay` and query/replay conflicts — exactly what the
/// long-running snapshot SELECTs here provoke). Gating on the receive LSN could let
/// the snapshot see a state older than `target_lsn`, reopening the data gap this
/// check exists to close.
///
/// Returns an error if the endpoint is a primary (NULL replay LSN). `timeout_ms = 0` skips.
pub async fn wait_for_snapshot_catchup(
    ...
    loop {
        let row = client
            ...
            .await
            .map_err(|e| anyhow::anyhow!("failed to query pg_last_wal_replay_lsn: {}", e))?;
        let snapshot_lsn: Option<PgLsn> = row.get(0);
        let snapshot_lsn = snapshot_lsn.ok_or_else(|| {
            anyhow::anyhow!(
                "snapshot.dedicated endpoint returned NULL for pg_last_wal_replay_lsn(). \
        ...
        if tokio::time::Instant::now() > deadline {
            return Err(anyhow::anyhow!(
                "Snapshot endpoint WAL at LSN {} has not caught up to CDC slot LSN {} after {}ms. \
                 Check pg_stat_replication on the CDC primary and pg_last_wal_replay_lsn() on \
