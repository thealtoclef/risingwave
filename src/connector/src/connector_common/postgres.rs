...
impl PostgresExternalTable {
    ...
    pub async fn connect(
        ...
        let (columns, pk_names, comments) = Self::discover_pk_and_full_columns(config, schema, table).await?;
...
pub async fn wait_for_snapshot_catchup(
    ...
    if timeout_ms == 0 {
        tracing::debug!("snapshot.catchup.timeout.ms=0, skipping WAL catch-up check");
        return Ok(());
    }
