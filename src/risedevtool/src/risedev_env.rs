...
pub fn generate_risedev_env_with_profile(
    ...
    for e in profile_env {
        ...
        if let Some((key, value)) = e.split_once('=') {
            match key {
                "SPANNER_PROJECT" => spanner_project = Some(value.to_owned()),
                "SPANNER_INSTANCE" => spanner_instance = Some(value.to_owned()),
                "SPANNER_DATABASE" => spanner_database = Some(value.to_owned()),
                "SPANNER_EMULATOR_HOST" => spanner_emulator_host = Some(value.to_owned()),
                _ => {}
            }
        }
    }
    ...
    let has_spanner_service = services
        .iter()
        .any(|s| matches!(s, ServiceConfig::Spanner(_)));
    if spanner_project.is_some() && !has_spanner_service {
        let project = spanner_project.unwrap_or_else(|| "test-project".to_owned());
        let instance = spanner_instance.unwrap_or_else(|| "test-instance".to_owned());
        let database = spanner_database.unwrap_or_else(|| "test-database".to_owned());
