...
use risingwave_connector::sink::iceberg::{
    COMMIT_CHECKPOINT_SIZE_THRESHOLD_MB, COMPACTION_DELETE_EQUALITY_RECORDS_COUNT_THRESHOLD,
    COMPACTION_DELETE_FILES_COUNT_THRESHOLD, COMPACTION_DELETE_POSITION_RECORDS_COUNT_THRESHOLD,
    ...
    ENABLE_PK_INDEX, ENABLE_SNAPSHOT_EXPIRATION, FORMAT_VERSION, ICEBERG_WRITE_MODE_COPY_ON_WRITE,
    ...
    SNAPSHOT_EXPIRATION_RETAIN_MAX, WRITE_MODE, parse_partition_by_exprs,
...
pub(crate) async fn resolve_iceberg_connection(
    ...
    let with_common = if iceberg_engine_connection.is_empty() {
        bail!(
            "to use iceberg engine {entity_kind}, please either set the session variable `iceberg_engine_connection` or specify `connection` in the WITH clause."
        );
    } else {
...
pub(crate) fn build_iceberg_sink_with_options(
    ...
    {
        let delete_position_records_count_threshold = delete_position_records_count_threshold
            .parse::<u64>()
            .map_err(|_| {
                ErrorCode::InvalidInputSyntax(format!(
                    "{} must be greater than 0: {}",
                    COMPACTION_DELETE_POSITION_RECORDS_COUNT_THRESHOLD,
                    delete_position_records_count_threshold
                ))
            })?;
