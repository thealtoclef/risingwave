...
impl DdlService for DdlServiceImpl {
    ...
    async fn create_iceberg_table(
        ...
        let mut fragment_graph = fragment_graph.ok_or_else(|| Status::invalid_argument("missing fragment_graph"))?;
        ...
        if fragment_graph.dependent_table_ids.len() != 1 {
            return Err(Status::invalid_argument("expected exactly 1 dependent table id"));
        }
        if !risingwave_common::catalog::TableId::from(fragment_graph.dependent_table_ids[0]).is_placeholder() {
            return Err(Status::invalid_argument("expected dependent table id to be a placeholder"));
        }
        ...
        let iceberg_source = iceberg_source.ok_or_else(|| Status::invalid_argument("missing iceberg_source"))?;
        let res = self
            .ddl_controller
    ...
    async fn create_iceberg_materialized_view(
        ...
        if let Err(err) = async {
            for _ in 0..5 {
                tokio::time::timeout(Duration::from_secs(30), self.barrier_scheduler
                    .run_command(database_id, Command::Flush))
                    .await
                    .map_err(|_| Status::deadline_exceeded("timed out waiting for MV backfill to converge"))??;
            }
            Ok(())
        }.await {
            let _ = self
            ...
            stream_graph_visitor::visit_fragment_mut(fragment, |node| {
                ...
                match node {
                    NodeBody::StreamScan(scan) => {
                        ...
                        if let Some(table) = &mut scan.arrangement_table {
                            if !risingwave_common::catalog::TableId::from(table.id)
                                .is_placeholder()
                            {
                                validation_error = Some(Status::invalid_argument(
                                    "expected StreamScan.arrangement_table.id to be a placeholder",
        ...
        // 4. create iceberg source
        let iceberg_source = iceberg_source
            .ok_or_else(|| Status::invalid_argument("missing iceberg_source"))?;
        let res = self
            .ddl_controller
            .run_command(DdlCommand::CreateNonSharedSource(iceberg_source))
            .await;
