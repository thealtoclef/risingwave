    ...
    pub async fn drop_object(
        ...
        for &job_id in &removed_streaming_job_ids {
            ...
            let obj_database_id = removed_objects
                .get(&obj_id)
                .unwrap_or_else(|| panic!("streaming job {job_id} not found in removed_objects"))
                .database_id
                .unwrap_or_else(|| panic!("streaming job {job_id} has no database_id"));
        ...
        let primary_state_table_ids: HashSet<TableId> = removed_state_table_ids
            ...
            .filter(|&table_id| {
                let obj_database_id = removed_objects
                    ...
                    .database_id
                    .unwrap_or_else(|| panic!("state table {table_id} has no database_id"));
                if obj_database_id == database_id {
                    true
                } else {
            ...
            .collect();
        // Every dropped state table, including cross-db ones, must be recorded in
        // `dropped_tables`: each database's `complete_dropped_tables` looks them up there to
        // notify hummock and compactors.
        let all_removed_state_table_ids: HashSet<TableId> = primary_state_table_ids
            .iter()
            .chain(cross_db_state_table_ids.values().flatten())
            .copied()
            .collect();
        ...
        for (cross_db_id, job_ids) in cross_db_job_map {
            ...
            let cross_state_table_ids = cross_db_state_table_ids
                .remove(&cross_db_id)
                .unwrap_or_default();
        ...
        let dropped_tables = removed_catalog_models
            ...
            .filter_map(|object_info| match object_info {
                PbObjectInfo::Table(table) if all_removed_state_table_ids.contains(&table.id) => {
                    Some(table)
                }
                _ => None,
            });
