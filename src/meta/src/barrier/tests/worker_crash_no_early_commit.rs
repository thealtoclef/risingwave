...
use risingwave_pb::stream_service::streaming_control_stream_response::{
    ReportPartialGraphFailureResponse, ResetPartialGraphResponse, Response,
};
...
impl GlobalBarrierWorkerContext for MockBarrierWorkerContext {
    ...
    fn mark_ready(&self, options: MarkReadyOptions) {
        let blocked_databases = match options {
            MarkReadyOptions::Global { blocked_databases } => blocked_databases,
            MarkReadyOptions::Database(_) => HashSet::new(),
        };
        self.0
            .send(ContextRequest::MarkReady(blocked_databases))
            .unwrap();
    }
...
#[tokio::test]
async fn test_bootstrap_recovery_partial_graph_reset_after_failure() {
    let (tx, mut rx) = mpsc::unbounded_channel();
    let context = Arc::new(MockBarrierWorkerContext(tx));
    let env = MetaSrvEnv::for_test_opts(MetaOpts::test(true), |_| {}).await;
    let (_request_tx, request_rx) = mpsc::unbounded_channel();
    let mut worker = GlobalBarrierWorker::new_inner(env, request_rx, context).await;
    ...
    let receive_initial_inject = async |request_rx: &mut UnboundedReceiver<
        StreamingControlStreamRequest,
    >| {
        let Request::CreatePartialGraph(req) = request_rx.recv().await.unwrap().request.unwrap()
        else {
    ...
    for request_rx in [&mut request_rx1, &mut request_rx2] {
        let Request::ResetPartialGraphs(reset_request) =
            request_rx.recv().await.unwrap().request.unwrap()
        else {
            unreachable!()
        };
