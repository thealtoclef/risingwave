...
impl DispatchExecutorInner {
    ...
    fn post_mutate_dispatchers(&mut self, mutation: &Option<Arc<Mutation>>) -> StreamResult<()> {
        ...
        if let Some(updates) = cdc_scan_actor_idleness(mutation) {
            ...
            for dispatcher in &mut self.dispatchers {
                changed.extend(dispatcher.update_data_suppression(&updates));
            }
            // An assignment barrier reaches every actor in the database, so log only where it
            // actually changed routing. Without it a silent change in data routing leaves no
            // trace to diagnose from.
            if !changed.is_empty() {
                let (suppressed, resumed): (Vec<_>, Vec<_>) =
                    changed.into_iter().partition(|(_, idle)| *idle);
                tracing::info!(
...
///
/// Correctness depends on that executor behaviour (see the invariant note in
/// `cdc_backill_v2.rs`): if it ever consumes data on an actor without splits, suppressing here
/// silently loses rows.
///
...
macro_rules! impl_dispatcher {
    ($( { $variant_name:ident } ),*) => {
        impl DispatcherImpl {
            ...
            pub fn update_data_suppression(
                &mut self,
                updates: &HashMap<ActorId, bool>,
            ) -> Vec<(ActorId, bool)> {
                match self {
...
trait Dispatcher: Debug + 'static {
    ...
    ///
    /// Returns the `(actor, suppressed)` transitions actually applied.
    fn update_data_suppression(
        &mut self,
        _updates: &HashMap<ActorId, bool>,
    ) -> Vec<(ActorId, bool)> {
        vec![]
    }
...
impl Dispatcher for BroadcastDispatcher {
    ...
    fn update_data_suppression(
        ...
        for (actor_id, suppressed) in updates {
            // An assignment names the scan actors of every CDC table in the database, so only
            // track our own outputs. Outputs joining on this barrier were already added in the
            // pre-mutate stage, and one added on a later barrier is only ever over-delivered to,
            // which the downstream tolerates.
            if !self.outputs.contains_key(actor_id) {
                continue;
            }
            let applied = if *suppressed {
                self.data_suppressed_actors.insert(*actor_id)
            } else {
                self.data_suppressed_actors.remove(actor_id)
            };
            if applied {
                changed.push((*actor_id, *suppressed));
            }
        }
        changed
    }
...
impl SimpleDispatcher {
    ...
    ///
    /// `chunk` must already have the output mapping applied: the `CdcFilter` predicate the names
    /// come from refers to columns of the downstream input, not of the upstream chunk.
...
impl Dispatcher for SimpleDispatcher {
    ...
    async fn dispatch_data(&mut self, chunk: StreamChunk) -> StreamResult<()> {
        // Filter after the output mapping, so `_rw_table_name` is read at the same column index
        // the downstream `CdcFilter` evaluates.
        let chunk = self.output_mapping.apply(chunk);
...
mod tests {
    ...
    async fn test_broadcast_dispatcher_data_suppression() {
        ...
        // Actors that are not outputs of this dispatcher, e.g. another CDC table's scan actors,
        // must not be tracked.
        assert!(
            dispatcher
                .update_data_suppression(&HashMap::from_iter([(ActorId::new(99), true)]))
                .is_empty()
        );
        assert!(
            !dispatcher
                .data_suppressed_actors
                .contains(&ActorId::new(99))
        );
    ...
    /// pass its tests and forward everything in the field. The edge's output mapping then drops
    /// `_row_id`.
    ...
    /// The output mapping keeps the first `columns` columns. CDC tests pass 3, as the real source
    /// to `CdcFilter` edge does, so the filter sees the mapped chunk it sees in production.
    ...
    async fn test_simple_dispatcher_routes_by_cdc_table_name() {
        let (mut dispatcher, mut rx) = simple_dispatcher_with(&["public.orders"], 3);
    ...
    async fn test_simple_dispatcher_accepts_every_name_in_the_set() {
        ...
        let (mut dispatcher, mut rx) =
            simple_dispatcher_with(&["public.\"Orders\"", "public.Orders"], 3);
    ...
    async fn test_simple_dispatcher_without_predicate_forwards_everything() {
        let (mut dispatcher, mut rx) = simple_dispatcher_with(&[], 3);
        ...
        let msg = rx.try_recv().unwrap();
        let chunk = msg.as_chunk().unwrap();
        assert_eq!(table_names_of(chunk), vec!["public.orders", "public.items"]);
    }
    ...
    /// The names come from the downstream `CdcFilter`, whose `InputRef` indexes the mapped chunk.
    /// A mapping that moves `_rw_table_name` must therefore not change which rows are routed.
    #[tokio::test]
    async fn test_simple_dispatcher_filters_after_output_mapping() {
        let (tx, mut rx) = channel_for_test();
        let mut dispatcher = SimpleDispatcher::new(
            Output::new(ActorId::new(1), tx),
            // Upstream `(offset, _rw_table_name, payload)` becomes downstream
            // `(payload, offset, _rw_table_name)`.
            DispatchOutputMapping::Simple(vec![2, 0, 1]),
            0.into(),
            ["public.orders".to_owned()].into_iter().collect(),
        );

        let mut offset = Utf8ArrayBuilder::new(2);
        let mut table_name = Utf8ArrayBuilder::new(2);
        let mut payload = Utf8ArrayBuilder::new(2);
        for name in ["public.orders", "public.items"] {
            offset.append(Some("0"));
            table_name.append(Some(name));
            payload.append(Some("{}"));
        }
        ...
        let msg = rx.try_recv().unwrap();
        let chunk = msg.as_chunk().unwrap();
        assert_eq!(table_names_of(chunk), vec!["public.orders"]);
    }
