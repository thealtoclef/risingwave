...
impl Serialize for Sstable {
    ...
    where
        S: serde::Serializer,
    {
        // foyer calls this twice per disk-cache insert — once through `bincode::serialized_size`
        // to size the entry, then again to write it. Cloning `SstableMeta` here therefore deep-
        // copied every `BlockMeta` twice per insert, so serialize out of borrowed state instead.
        let filter_data;
