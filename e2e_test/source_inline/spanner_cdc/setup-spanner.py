...
# Configuration
# ---------------------------------------------------------------------------
SPANNER_PROJECT = "rainbow-data-production-483609"
SPANNER_INSTANCE = "rainbow-spanner"
SPANNER_DATABASE = "rainbow"
...
def log_credentials():
    ...
    if cred_file:
        ...
        print("  GOOGLE_APPLICATION_CREDENTIALS is not set — using ADC (gcloud auth application-default login)")
...
def step_cleanup(database):
    ...
    for table in ["orders", "products", "users"]:
        try:
            execute_ddl(database, [f"DROP TABLE IF EXISTS {table}"], f"Drop table {table}")
        except Exception as e:
            print(f"  (ignored: {e})")
...
def main():
    print(f"Spanner setup for RisingWave CDC e2e test")
